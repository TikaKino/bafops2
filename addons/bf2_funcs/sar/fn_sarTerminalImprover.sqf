...
if (isServer) then {
	...
	if (isNil "_terminalObject" || isNil "_beaconLogic") then {
		systemChat format ["Null terminal or beacon to CreateImproveTerminal: %1, %2",_terminalObject,_beaconLogic];
	} else {
		...
		[
			...
			"Connect Terminal",										// Title of the action
			...
			{ 													// Code executed on completion
				...
				["Terminal connected. Beacon accuracy improved."] remoteExec ["systemChat",_caller];
				[_arguments select 0] remoteExecCall ["bafops2_fnc_sarBeaconImprove",2];
